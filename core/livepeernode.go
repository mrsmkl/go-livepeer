...
func NewLivepeerNode(e eth.LivepeerEthClient, vn net.VideoNetwork, nodeId NodeID, wd string, dbh *common.DB) (*LivepeerNode, error) {
	...
	return &LivepeerNode{VideoCache: NewBasicVideoCache(vn), VideoNetwork: vn, Identity: nodeId, Eth: e, WorkDir: wd, Database: dbh, EthServices: make(map[string]eth.EventService), ClaimManagers: make(map[int64]eth.ClaimManager), SegmentChans: make(map[int64]SegmentChan), mutex: &sync.Mutex{}}, nil
}
...
func (n *LivepeerNode) TranscodeSegment(job *ethTypes.Job, ss *SignedSegment) {
	...
	//go func() { ch <- segChan }() // send segment to transcode loop
	select {
	case ch <- segChan:
